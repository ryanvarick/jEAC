...
public class RuleCrossoverPipeline extends BreedingPipeline
    ...
    public void setup(final EvolutionState state, final Parameter base)
        ...
        ruleCrossProbability = state.parameters.getFloatWithDefault(base.push(P_PROB),
                                                                    def.push(P_PROB),0.5f);
        if (ruleCrossProbability > 1.0 || ruleCrossProbability < 0.0) 
            state.output.fatal("Rule cross probability must be between 0 and 1",base.push(P_PROB),
                               def.push(P_PROB));
        }
