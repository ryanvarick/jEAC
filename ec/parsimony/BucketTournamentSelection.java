...
public class BucketTournamentSelection extends SelectionMethod implements SteadyStateBSourceForm
    ...
    public void setup(final EvolutionState state, final Parameter base)
        ...
        if( state.parameters.exists( base.push(P_SIZE) ) ||
            state.parameters.exists( def.push(P_SIZE) ) )
            {
            nBuckets = state.parameters.getInt(base.push(P_SIZE),def.push(P_SIZE),1);
            if (nBuckets < 1)
                {
                state.output.fatal("The number of buckets size must be >= 1.",base.push(P_SIZE),def.push(P_SIZE));
                }
            }
        else
            {
