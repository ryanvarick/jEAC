...
public class RandomBranch extends GPNodeBuilder 
    ...
    private GPNode randomBranch(final EvolutionState state,
        ...
        if (maxLength == 1)         // pick a random terminal
            ...
            GPNode n = (GPNode)(nn[state.random[thread].nextInt(nn.length)].node.clone());
            ...
            GPFuncInfo[] okayNonterms = set.nonterminalsUnderArity[type.type][len];

            if (okayNonterms.length == 0) // no nodes, pick a nonterminal
                {
                GPFuncInfo[] okayTerms = set.terminals[type.type];
                GPNode n = (GPNode)(okayTerms[state.random[thread].nextInt(okayTerms.length)].node.clone());
                n.resetNode(state,thread);  // give ERCs a chance to randomize
                n.argposition = (byte)argposition;
                n.parent = parent;
                return n;
                ...
                {
                GPNode n = (GPNode)(okayNonterms[state.random[thread].nextInt(okayNonterms.length)].node.clone());
                n.resetNode(state,thread);  // give ERCs a chance to randomize
                n.argposition = (byte)argposition;
                n.parent = parent;
