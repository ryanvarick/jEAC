...
public class GPTreeConstraints implements Clique
    ...
    public final void setup(final EvolutionState state, final Parameter base)
        ...
        while (e.hasMoreElements())
            ...
            GPFuncInfo[] i = functionset.nodes[t.type];
            if (i.length==0) // yeesh
    ...
    private void checkFunctionSetValidity(final EvolutionState state,
        ...
        for (int x=0; x<i.length;x++)
            for (int y=0;y<i[x].node.constraints(initializer).childtypes.length;y++)
                if (done.get(i[x].node.constraints(initializer).childtypes[y])==null)
                    {
                    checkFunctionSetValidity(
                        state, done, i[x].node.constraints(initializer).childtypes[y]);
                    }
        }
